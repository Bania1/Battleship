...
int colocarBarco(Cuadricula* cuadricula, Barco* barco) {
    ...
    if (orientacion == 0) {
        for (int i = 0; i < barco->tamano; i++) {
            ...
            //guardar la coordenada del barco
            //barco->id = ;
    ...
    barco->coordenada->coordenada[fila][columna] = fila,columna; //{fila,columna}
...
int disparar(Cuadricula* cuadricula_disparo, Cuadricula* cuadricula_barco, Barco barcos[], char* coordenadas, int turno) {
    ...
    if (cuadricula_barco->tablero[fila][columna] == 'B') {
        ...
        for (int i = 0; i < 5; i++) {
            if (barcos[i].simbolo == 'B' && barcos[i].coordenada->coordenada[fila][columna] == fila,columna) {
                barcos[i].salud--;
                if (barcos[i].salud == 0) {
    ...
    do {
        ...
        switch (opcion) {
            case 1:
                ...
                Barco barcos[] = {{4, 'B', 0,{0}}, {3, 'B', 0,{0}}, {3, 'B', 0,{0}}, {2, 'B', 0,{0}}, {2, 'B', 0,{0}}};
